<div>
            ...
            <div class="row align-items-center">
                <div class="border-0 mb-4">
                    <div class="card-header py-3 no-bg bg-transparent d-flex align-items-center px-0 justify-content-between border-bottom flex-wrap">
                        <h3 class="fw-bold mb-0">Danh Sách Ngành</h3>
                ...
                <div class="col-sm-12">
                    <div class="card mb-3">
                        ...
                        <div class="row">
                            ...
                            <table class="table table-hover align-middle mb-0" style="width:100%">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td>{{$i}}</td>
                                        <td>
                                            <a href="#" class="fw-bold text-secondary">{{$nganh->MANGANH}}</a>
                                        </td>
                                        <td>
                                            <span class="fw-bold ms-1">{{$nganh->TENNGANH}}</span>
                                        </td>
                                        <td>
                                            <span class="fw-bold ms-1">{{$nganh->khoa->TENKHOA}}</span>
                                        </td>
                                        <td>{{$nganh->bacdaotao->TENDAYDU}}</td>
                                        <td>{{$nganh->hedaotao->TENDAYDU }}</td>
                                        <td>
                                            <div class="btn-group" role="group" aria-label="Basic outlined example">
                                                <button type="button" class="btn btn-outline-secondary" wire:click.prevent="edit_branch({{$nganh->ID_NGANH}})">
                                                    <i class="icofont-edit text-success"></i>
                                                </button>
                                                <button type="button" class="btn btn-outline-secondary deleterow" onclick="confirm('Bạn có chắc chắn muốn xoá không?') || event.stopImmediatePropagation()" wire:click.prevent="delete_branch({{$nganh->ID_NGANH}})">
                            ...
                            {{ $nganhs->links('livewire.livewire-pagination-link') }}
                        </div>
                    </div>
                </div>
            </div><!-- Row End -->
    ...
    <div class="modal-dialog modal-dialog-centered modal-md modal-dialog-scrollable">
         <div class="modal-content" style="overflow: scroll;">
             <div class="modal-header">
                 <h5 class="modal-title  fw-bold" id="leaveaddLabel"> Cập nhật ngành</h5>
                 <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
             <form method="POST" wire:submit.prevent="update_branch()">
                 <div class="modal-body">
                     <div class="mb-3">
                         <label for="sub" class="form-label">Khoa</label>
                     ...
                     <div class="col">
                         <label for="depone" class="form-label">Mã ngành</label>
                         <input type="text" class="form-control" id="depone" wire:model="MANGANH">
                         @error('MANGANH') <p class="text-danger">{{ $message }}</p> @enderror
                     </div>
                     <div class="deadline-form mt-3">
                         <div class="row g-3 mb-3">
                             <div class="col">
                                 <label for="deptwo" class="form-label">Tên ngành</label>
                                 <input type="text" class="form-control" id="deptwo" wire:model="TENNGANH">
                                 @error('TENNGANH') <p class="text-danger">{{ $message }}</p> @enderror
                             </div>
                         </div>
                         <div class="row g-3 mb-3">
                             <div class="col">
    ...
    <div class="modal fade" id="modalAdd" tabindex="-1" aria-hidden="true" wire:ignore.self>
     <div class="modal-dialog modal-dialog-centered modal-md modal-dialog-scrollable">
         <div class="modal-content" style="overflow: scroll;">
             <div class="modal-header">
                 <h5 class="modal-title  fw-bold" id="leaveaddLabel"> Thêm ngành mới</h5>
             </div>
             <form method="POST" wire:submit.prevent="add_branch()">
             <div class="modal-body">
                     <div class="mb-3">
                         <label for="sub" class="form-label">Khoa</label>
                         <select class="form-control" id="" wire:model="ID_KHOA">
